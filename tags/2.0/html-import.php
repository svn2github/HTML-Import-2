...
function html_import_css() {
	$options = get_option('html_import'); ?>
		...
		/* jQuery tabs */
		.ui-tabs-nav { border-bottom: 1px solid #ccc; height: 27px; margin: 20px 0; padding: 0; }
		.ui-tabs-nav li { display: block; float: left; margin: 0; }
		.ui-tabs-nav li a { padding: 4px 20px 6px; font-weight: bold; }
		.ui-tabs-nav li a { border-style: solid; border-color: #CCC #CCC #F9F9F9; border-width: 1px 1px 0; color: #C1C1C1; text-shadow: rgba(255, 255, 255, 1) 0 1px 0; display: inline-block; padding: 4px 14px 6px; text-decoration: none; margin: 0 6px -1px 0; -moz-border-radius: 5px 5px 0 0; -webkit-border-top-left-radius: 5px; -webkit-border-top-right-radius: 5px; -khtml-border-top-left-radius: 5px; -khtml-border-top-right-radius: 5px; border-top-left-radius: 5px; border-top-right-radius: 5px; }
		.ui-tabs-nav li.ui-tabs-selected, .ui-tabs-nav li.ui-state-active { padding-bottom: 1px; background: #fff; }
		.ui-tabs-nav li.ui-tabs-selected a, .ui-tabs-nav li.ui-state-active a { border-width: 1px; color: #464646; }
		.ui-tabs-panel { clear: both; }
		.ui-tabs-panel h3 { font: italic normal normal 24px/29px Georgia,"Times New Roman","Bitstream Charter",Times,serif; margin: 0; padding: 0 0 5px; line-height: 35px; text-shadow: 0 1px 0 #fff; }
		.ui-tabs-panel h4 { font-size: 15px; font-weight: bold; margin: 1em 0; }
		.wrap h3 /*, .wrap div.ui-tabs-panel */ { display: none; }
		</style>
	<?php
}
...
function html_import_add_pages() {
	...
	add_action( 'admin_head-'.$pg, 'html_import_css');
...
function html_import_options_page() {
		 ...
		 ?> 
			...
			<p><small><?php _e('if you need to redirect visitors from the old file locations to your new WordPress pages, copy these redirects 
				...
				<textarea id="import-result"><?php
					$url = esc_url(rtrim($options['old_url'], '/'));
    ...
    <div class="wrap" id="html_import">
		<h2><?php _e( 'HTML Import Options ', 'import-html-pages'); ?></h2> 
	...
	echo '<div class="ui-tabs">
		...
		foreach ( array('Files', 'Content', 'Title', 'Metadata', 'Taxonomies') as $section )
	...
	<div class="ui-tabs-panel" id="files">
	...
	<div class="ui-tabs-panel" id="content">
    ...
    <div class="ui-tabs-panel" id="title">
	    ...
	    <input type="text" name="remove_from_title" id="remove_from_title" value="<?php esc_attr_e($options['remove_from_title']); ?>" 
	    	class="widefloat" />  </label><br />
		<small><?php _e("Any common title phrase (such as the site name, which most themes will print automatically)", 'import-html-pages'); ?></small></p>
    </div>
    

    <h3><?php _e("Metadata", 'import-html-pages'); ?></h3>
    <div id="metadata" class="ui-tabs-panel" id="metadata">
		...
		<div id="custom-taxonomy" class="clear">
				<?php foreach ( $wp_taxonomies as $tax ) : ?>
					<?php if (!in_array($tax->name, $standardtaxes)) : 
						...
						echo '<p class="clear"><label>'.$tax->label.'<br />
	...
	-->
		...
		<p><?php _e('Assign categories, tags, and custom taxonomy terms to your imported posts:', 'import-html-pages'); ?></p>
				<?php foreach ( $wp_taxonomies as $tax ) :
				if (!in_array($tax->name, array('nav_menu', 'link_category', 'post_format'))) : ?>
					<?php 
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			...
			//$(".wrap h3, .wrap .ui-tabs-panel").show();
		});
	</script>    
	</div> <!-- .wrap -->
